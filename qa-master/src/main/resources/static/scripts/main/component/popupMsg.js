        ...
        _tpl: [
            '<div class="zh-add-question-form" style="margin:0;">',
                '<div class="zg-section-big clearfix">',
                    '<div class="add-question-section-title">Send to:</div>',
                    '<div class="zg-form-text-input add-question-title-form" style="position: relative;">',
                        '<input type="text" class="js-name zg-editor-input zu-seamless-input-origin-element" placeholder="Name" style="height:22px;min-height:auto;"></textarea>',
                    '</div>',
                '</div>',
    ...
    function fStaticShow(oConf) {
        ...
        var oPopup = new Popup({
            ...
            ok: function () {
                ...
                var oData = oAdd.val();
                if (!oData.toName) {
                    that.error('Please fill in the name');
                    return true;
                }
                if (!oData.content) {
                    that.error('Please fill in the private message content');
                    return true;
                }
                // 避免重复提交
                if (bSubmit) {
